...
Health check (fast and does not force model download at startup):
```bash
...
3. Railway builds from `Dockerfile` automatically and binds to `$PORT` (already handled in this repo).
4. In Railway, enable **Public Networking** (otherwise it stays *Unexposed service*).
5. After deploy, open logs and verify you see request lines like `GET /health -> 200`.
6. Then use your public domain:
...
- If deployment shows **Unexposed service**, enable Public Networking for the service.
- If startup fails, check Deploy Logs for Python package build errors (torch wheel).
- `/health` should return quickly even before the model loads.
- The model is loaded lazily on first `/image-to-3d` request, so cold start is expected once.
